...
function BlogViewContent() {
  ...
  useEffect(() => {
    ...
    fetch('/api.php?type=blogs')
      .then(r => r.json())
      .then(posts => { setPost(posts.find(p => p.id === id) || null); setLoading(false); })
    ...
    <>
      <section className="bg-gradient-to-br from-primary-950 to-primary-800 py-14">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <span className="inline-block bg-white/10 text-primary-200 text-xs font-semibold px-3 py-1 rounded-full border border-white/10 mb-4">{post.category}</span>
      ...
      <section className="py-12 bg-slate-50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div className="bg-white rounded-3xl border border-slate-100 p-6 sm:p-10">
            {post.excerpt && (
              <p className="text-slate-600 text-base leading-relaxed border-l-4 border-primary-400 pl-5 mb-8 italic">{post.excerpt}</p>
            )}
            <div className="text-slate-700 text-sm leading-relaxed whitespace-pre-line">{post.content}</div>
          </div>
          <div className="mt-8">
